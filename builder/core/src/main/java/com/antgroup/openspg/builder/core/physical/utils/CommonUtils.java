...
public class CommonUtils {
    ...
    public static ProjectSchema getProjectSchema(BuilderContext context) {
        HttpClientBootstrap.init(
                new ConnectionInfo(context.getSchemaUrl())
                        .setConnectTimeout(6000)
                        .setReadTimeout(600000)
        );

        SchemaFacade schemaFacade = new HttpSchemaFacade();
        ApiResponse<ProjectSchema> response = schemaFacade.queryProjectSchema(
                new ProjectSchemaRequest(context.getProjectId()));
    ...
    public static List<BaseSPGRecord> convertNodes(
        ...
        for (SubGraphRecord.Node node : resultNodes) {
            ...
            Map<String, String> stringMap = node.getProperties().entrySet()
                    .stream()
    ...
    public static List<BaseSPGRecord> convertEdges(
        ...
        for (SubGraphRecord.Edge edge : resultEdges) {
            RelationIdentifier identifier = RelationIdentifier.parse(
            ...
            Map<String, String> stringMap = edge.getProperties().entrySet().stream()
                    .collect(Collectors.toMap(
                            Map.Entry::getKey,
                            entry -> entry.getValue() == null ? null: entry.getValue().toString())
                    );
    ...
    public static PemjaConfig getSplitterConfig(
        ...
        return new PemjaConfig(
                pythonExec,
                pythonPaths,
                pythonEnv,
                hostAddr,
                projectId,
                splitter, Maps.newHashMap()
        );
    }
}
