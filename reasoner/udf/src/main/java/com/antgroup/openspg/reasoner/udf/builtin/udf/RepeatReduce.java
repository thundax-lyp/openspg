...
import com.antgroup.openspg.reasoner.udf.builtin.ReasonerCommonUtils;
...
public class RepeatReduce {
    ...
    public Object reduce(
        ...
        Map<String, Object> contextMap = ReasonerCommonUtils.getParentContext(context);
        for (int i = 0; i < itemList.size(); ++i) {
            Object cur = itemList.get(i);
            Map<String, Object> subContext = new HashMap<>(contextMap);
            subContext.put(preName, preValue);
            subContext.put(curName, ReasonerCommonUtils.getRepeatItemContext(cur));
            preValue = RuleRunner.getInstance().executeExpression(subContext, Lists.newArrayList(express), "");
        }
        return preValue;
    }
