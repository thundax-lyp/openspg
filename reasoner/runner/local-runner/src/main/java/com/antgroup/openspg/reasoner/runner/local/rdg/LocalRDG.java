...
public class LocalRDG extends RDG<LocalRDG> {
    ...
    private java.util.Map<IVertexId, String> getStartIdWithHitRuleValue(
        ...
        for (KgGraph<IVertexId> kgGraph : kgGraphList) {
            ...
            for (Tuple2<String, java.util.List<String>> field : fields) {
                ...
                java.util.Map<String, Object> values = (java.util.Map<String, Object>) context.get(field._1);
    ...
    private java.util.Set<IVertexId> getRemoveHashSet(
        ...
        java.util.Set<IVertexId> updateSet = new HashSet<>(set1);
    ...
    public LocalRDG patternScan(Pattern pattern) {
        ...
        log.info("LocalRDG patternScan,,pattern={},rule={},edgeRule={}",
                pattern, JSON.toJSONString(rootVertexRuleList), JSON.toJSONString(edgeRuleMap));
        ...
        for (KgGraph<IVertexId> kgGraphId : this.kgGraphList) {
            ...
            Callable<KgGraph<IVertexId>> patternScanCallable = () -> patternMatcher.patternMatch(
                    id,
                    null,
                    null,
                    pattern,
        ...
        for (Future<KgGraph<IVertexId>> future : futureList) {
            ...
            if (Utils.randomLog()) {
                log.info("LocalRDG PatternScan,kgGraph={}", kgGraph.getVertex(pattern.root().alias()).get(0).getId());
            }
            count++;
        ...
        log.info("LocalRDG patternScan,root={},matchCount={} cost time={}", pattern.root(), count, System.currentTimeMillis() - startTime);
    ...
    public LocalRDG linkedExpand(EdgePattern<LinkedPatternConnection> pattern) {
        ...
        for (KgGraph<IVertexId> kgGraph : this.kgGraphList) {
            Callable<java.util.List<KgGraph<IVertexId>>> linkExpandCallable = () -> linkEdge.link(kgGraph);
            if (null != callableWrapper) {
        ...
        log.info("LinkedExpand, funcName={},matchCount={}, linkedTargetVertexSize={} cost time={}", pattern.edge().funcName(), count, targetVertexSize, System.currentTimeMillis() - startTime);
        this.executionRecorder.stageResultWithDetail(
    ...
    private Future<java.util.List<KgGraph<IVertexId>>> processKgGraphWithSameRoot(
        ...
        Callable<java.util.List<KgGraph<IVertexId>>> expandIntoCallable = () -> {
            ...
            if (CollectionUtils.isNotEmpty(intersectionAliasSet)) {
                edgeValidTargetIdSet = RunnerUtil.getEdgeAlias2ValidTargetIdMap(intersectionAliasSet, sameRootKgGraphList, matchPattern);
            }

            KgGraph<IVertexId> matchedKgGraph = patternMatcher.patternMatch(
                    rootId,
                    null,
                    null,
                    matchPattern,
            ...
            for (KgGraph<IVertexId> kgGraph : sameRootKgGraphList) {
                ...
                if (null != maxPathLimit && count >= maxPathLimit) {
                    log.warn("ExpandInto,pathLimit,count={},matchCount={},id={}", count, matchCount, rootId);
                    break;
                }
            }
            return result;
    ...
    public LocalRDG expandInto(PatternElement target, Pattern pattern) {
        ...
        log.info("LocalRDG expandInto,,pattern={},rule={},edgeRule={}", pattern, JSON.toJSONString(vertexRuleList), JSON.toJSONString(edgeRuleMap));
        ...
        log.info("LocalRDG ExpandInto,patternRoot={},matchCount={} cost time={}", pattern.root(), count, System.currentTimeMillis() - startTime);
        this.executionRecorder.stageResultWithDetail(
    ...
    public LocalRDG filter(Rule rule) {
        ...
        KgGraphSplitStaticParameters staticParameters = new KgGraphSplitStaticParameters(vertexSet, this.kgGraphSchema);
        java.util.List<KgGraph<IVertexId>> newKgGraphList = new ArrayList<>();
        long count = 0;
        for (KgGraph<IVertexId> kgGraph : this.kgGraphList) {
        ...
        java.util.Set<IVertexId> failedStartIdSet = getRemoveHashSet(originStartId, afterStartId);
        this.kgGraphList = newKgGraphList;
        log.info("Filter,rule={},matchCount={}", exprStringSet, count);
        this.executionRecorder.stageResultWithDetail(
        ...
        if (null == aggregations || aggregations.isEmpty()) {
            ...
            kgGraphListProcess =
                    new KgGraphListProcess() {
                        @Override
                        public java.util.List<KgGraph<IVertexId>> reduce(Collection<KgGraph<IVertexId>> kgGraphs) {
                            KgGraphImpl kgGraph = new KgGraphImpl();
                            assert kgGraphs instanceof java.util.List;
                            kgGraph.merge(kgGraphs, kgGraphSchema);
            ...
            KgGraphAggregateImpl impl =
                    new KgGraphAggregateImpl(
                            ...
                            this.maxPathLimit);
            impl.init();
            kgGraphListProcess = kgGraphs -> Lists.newArrayList(impl.map(kgGraphs));
        }
    ...
    public LocalRDG addFields(Map<Var, Expr> fields) {
        ...
        JavaConversions.mapAsJavaMap(fields)
                .forEach((var, expr) -> addFieldsInfo.put(var, WareHouseUtils.getRuleList(expr)));
    ...
    private void addProperty(String alias, Field property, Boolean isLast) {
        ...
        this.executionRecorder.stageResultWithDesc(
                ...
                count,
                "addProperty(" + alias + "." + property + ")");
        log.info("LocalRDG.addProperty,sinkCount={} addProperty={}.{}", count, alias, property);
    }
    ...
    private void addRelation(AddPredicate addPredicate) {
        ...
        log.info("LocalRDG.addRelation,sinkCount={} addPredicate={}", sinkCount, addPredicate);
    }
    ...
    public LocalRDG join(
        ...
        try {
            ...
            if (!diffEdgeDirectionMap.isEmpty()) {
                ...
                left = left.stream()
                        .map(kgGraph -> edgeDirection.convert((KgGraphImpl) kgGraph))
                        .collect(Collectors.toList());
            }
    ...
    private java.util.List<java.util.Map<String, Object>> generateStartNodeDebugInfo() {
        ...
        for (IVertexId id : debugInfoMap.keySet()) {
            ...
            IVertex<IVertexId, IProperty> vertex =
                    graphState.getVertex(debugInfoWithStartId.getVertexId(), null);
            if (vertex == null) {
                continue;
            }
            debugStartNodeINfos.add(
                    getProcessInfo(
                            debugInfoWithStartId,
                            vertex.getValue().get(Constants.NODE_ID_KEY).toString(),
                            "",
                            ""));
        }
    ...
    public LocalReasonerResult getResult() {
        java.util.List<java.util.Map<String, Object>> debugStartNodeINfos =
                generateStartNodeDebugInfo();
    ...
    private LocalReasonerResult getRDGGraph() {
        ...
        this.kgGraphList.forEach(graph -> {
