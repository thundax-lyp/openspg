...
public class KagExtractorAsyncTask extends AsyncTaskExecuteTemplate {
    ...
    @Autowired
    private MemoryTaskServer memoryTaskServer;
    ...
    private static class ExtractorTaskCallable extends MemoryTaskServer.MemoryTaskCallable<String> {
        ...
        class ExtractTaskCallable implements Callable<List<SubGraphRecord>> {
            ...
            public List<SubGraphRecord> call() throws Exception {
                ...
                List<SubGraphRecord> records = JSON.parseObject(JSON.toJSONString(result), new TypeReference<List<SubGraphRecord>>() {
                });

                for (SubGraphRecord subGraphRecord : records) {
                    int nodes = CollectionUtils.isEmpty(subGraphRecord.getResultNodes())
                            ? 0
                            : subGraphRecord.getResultNodes().size();
                    int edges = CollectionUtils.isEmpty(subGraphRecord.getResultEdges())
                            ? 0
                            : subGraphRecord.getResultEdges().size();
                    addTraceLog("Extract chunk(%s:%s) successfully. nodes:%s. edges:%s", chunk.getName(), chunk.getShortId(), nodes, edges);
                }
                return records;
            }
        }
